 ...
 *
 * The single LinkedIn source. Writes data/_generated/linkedin.json, which the
 * data layer reads. Hand-authored corrections go in data/overrides.json and are
 * re-applied on every run, so a re-sync never clobbers them.
 *
...
async function main() {
  ...
  // Hand-authored corrections. data/_generated/ is overwritten on every run, so
  // fixes live in data/overrides.json and get re-applied here instead of being
  // lost the next time someone syncs.
  const applied = [];
  let overrides = null;
  try {
  ...
  if (overrides?.experience) {
    for (const role of out.experience) {
      const patch = overrides.experience[role.company];
      if (!patch) continue;
      for (const [k, v] of Object.entries(patch)) {
        if (k.startsWith("_")) continue;
        if (role[k] !== v) { role[k] = v; applied.push(`${role.company}.${k}`); }
      }
    }
  }
  if (overrides?.skills?.add?.length) {
    const have = new Set(out.skills.map((s) => s.name));
    for (const name of overrides.skills.add) {
      if (!have.has(name)) { out.skills.push({ name }); applied.push(`skills+${name}`); }
    }
  }
  ...
  if (untitled.length) {
    ...
    console.log(`    Add one under "experience" in data/overrides.json —`);
    console.log(`    the site can't render "null at Company".`);
  }
}
